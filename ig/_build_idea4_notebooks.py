...
L5 = [
    ...
    md("## Step 2 — Encode, then integrate\n\n"
       ...
       "the all-zero embedding ('absence of signal'). We use **200 steps** — the "
       "smoke test used 48 and the completeness delta was large; more steps "
       "tightens it."),
    ...
    md("## Caveats (carry these into lesson 7)\n\n"
       "- **Baseline dependence.** IG attributions change with the baseline; we "
       "report the zero-embedding baseline.\n"
       "- **Completeness.** If the delta above isn't small, raise `n_steps`.\n"
       "- **No interactions.** IG scores residues individually; it won't tell "
       "you that residue *i* matters *because* it pairs with residue *j* — a "
       "real limitation for interface hotspots, which are often cooperative."),
]
